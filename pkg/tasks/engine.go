...
// A schedule whose action fails is NOT re-anchored (see sweepSchedules), so
// it stays due and retries on every 5s tick: unthrottled that is ~17k
// identical lines per day per broken schedule. At failReportThrottle=60 the
// sweeper's heartbeat is one line per ≈5 minutes, so any 15-minute dashboard
// window still holds at least three.
